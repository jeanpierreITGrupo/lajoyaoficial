...
class AccountMoveLineImport(models.TransientModel):
    ...
    def _input_fields(self):
        ...
        res = {
            ...
            'type_document_id' :{'method': self._handle_type_document},
            'means_payment_id':{'method': self._handle_means_payment},
            'fefectivo_id':{'method': self._handle_fefective},
        }
        return res
    ...
    def _process_header(self, header_fields):
        ...
        print "cabezeras2", header_fields
    ...
    def _handle_type_document(self, field, line, move, aml_vals):
        print "hola 3"
        if not aml_vals.get('type_document_id'):
            print "hola 4"
            ana_mod = self.env['it.type.document']
            input = line[field]
            domain = []
            analytic_accounts = ana_mod.search(
                domain + [('code', '=', input)])
            if len(analytic_accounts) == 1:
                aml_vals['type_document_id'] = analytic_accounts.id
            if not analytic_accounts:
    ...
    def _handle_means_payment(self, field, line, move, aml_vals):
        print "hola 3"
        if not aml_vals.get('means_payment_id'):
            print "hola 4"
            ana_mod = self.env['it.means.payment']
            input = line[field]
            domain = []
            analytic_accounts = ana_mod.search(
                domain + [('code', '=', input)])
            if len(analytic_accounts) == 1:
                aml_vals['means_payment_id'] = analytic_accounts.id
            if not analytic_accounts:

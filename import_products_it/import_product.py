...
class import_product(models.Model):
	...
	def correct_data(self):
		for line in self.lines:
			if line.state == 'invalid':
				product = self.env['product.product'].search([('default_code','=',line.code.strip(" "))])
				if product:
					line.state = 'valid'
	...
	def import_data(self):
		for line in self.lines2:
			...
			route_ids = self.env['stock.location.route'].search([('name','=',line.routes_id)])
			vals = {

    ...
    def _check_analytic_required_msg(self, cr, uid, ids, context=None):
        for move_line in self.browse(cr, uid, ids, context):
            ...
            if analytic_policy == 'always' and \
                    not move_line.analytic_account_id and  not move_line.analytics_id:
                return _("LA CUENTA ANALITICA ES OBLIGATORIA PARA EL TIPO CONTABLE CONFIGURADO EN LA CUENTA")
				
            elif analytic_policy == 'never' and \
                    (move_line.analytic_account_id or move_line.analytics_id):
                return _("NO SE PERMITE CONSIGNAR CUENTA O DISTRIBUCION ANALITICA PARA EL TIPO CONTABLE AL QUE PERTENECE LA CUENTA REGISTRADA")

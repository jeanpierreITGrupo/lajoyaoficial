...
class hr_tareo_line(models.Model):
	...
	num_days_subs                  = fields.Integer(u'Días subsidiados por Enfermedad')
	num_days_subs_mater            = fields.Integer(u'Días subsidiados por Maternidad')
...
class hr_tareo(models.Model):
	...
	def make_pdf(self, htl_id, digital_sgn):
		...
		for tareo_line in self.env['hr.tareo.line'].search([('id','in',htl_id)]).sorted(key=lambda r: r.employee_id.codigo_trabajador):
			...
			data.append([u'Categoría:','',(tareo_line.employee_id.tipo_trabajador.name if tareo_line.employee_id.tipo_trabajador.name else ''),'',u'Días Subsidiados por Enfermedad:','',str(tareo_line.num_days_subs if tareo_line.num_days_subs else '0'),u'Fecha de Ingreso:',(ff_ing if tareo_line.employee_id.fecha_ingreso else '')])
			estilo.append(('SPAN',(0,row_pos),(1,row_pos)))
			estilo.append(('SPAN',(2,row_pos),(3,row_pos)))
			estilo.append(('SPAN',(4,row_pos),(5,row_pos)))
			estilo.append(('ALIGN',(0,row_pos),(8,row_pos),'LEFT'))
			...
			data.append([u'Régimen Pensionario:','',(tareo_line.employee_id.afiliacion.name if tareo_line.employee_id.afiliacion.name else ''),'',
				u'Días no Subsidiados o Permisos:','',str(tareo_line.num_days_not_subs if tareo_line.num_days_not_subs else '0'),
			...
			data.append([u'CUPSS','',(tareo_line.employee_id.cusspp if tareo_line.employee_id.cusspp else ''),'',
				...
				u'Días Feriados Trabajados',str(int(tareo_line.dias_feriados_trabajados) if tareo_line.dias_feriados_trabajados else '0')])
			estilo.append(('SPAN',(0,row_pos),(1,row_pos)))
			estilo.append(('SPAN',(2,row_pos),(3,row_pos)))
			estilo.append(('SPAN',(4,row_pos),(5,row_pos)))
			estilo.append(('ALIGN',(0,row_pos),(8,row_pos),'LEFT'))

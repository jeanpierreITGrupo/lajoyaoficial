...
class AccountCSVExport(orm.TransientModel):
    ...
    def _get_rows_journal_entries(self, cr, uid, ids,
        ...
        SELECT
		  ...
		  account_tax_code.name AS aml_tax_code_code,
		  ...
		  account_move_line.nro_comprobante AS nro_comprobante,
		  res_partner.name AS partner_name,
		  account_move_line.name AS description,              
          account_move_line.ref AS ref,
		  account_move.state AS estado,
          t8.name as motivo,
          CASE WHEN account_invoice.is_employee = true then he.name_related else rp.name end
          as
          beneficiario,
		  fj.name||'-'||fj.concepto as flujo_caja,
          U1.usuario     
        ...
        FROM
          ...
          LEFT JOIN account_invoice on
            (account_invoice.move_id = account_move.id)
          LEFT JOIN motivo_reparables t8 on t8.id = account_invoice.motivo_reparables_id
          

          LEFT JOIN hr_employee he on he.id = account_invoice.b_employee_id 
          LEFT JOIN res_partner rp on rp.id = account_invoice.b_partner_id
		  LEFT JOIN it_flujo_caja fj on fj.id= account_move_line.flujo_caja_id
          left join res_users U1 on U1.id=account_move.write_uid

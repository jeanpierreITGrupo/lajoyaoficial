...
instance.web.form.CompletionFieldMixin = {
    ...
    get_search_result: function(search_val) {
        ...
        if (this.field.type == "many2one")
        {
            ...
            {
                if (this.view.model == "account.invoice" || this.view.model == "account.invoice.line" || this.view.model == "account.move" || this.view.model == "account.move.line" || this.view.model == "product.product" || this.view.model == "product.template" || this.view.model == "product.category" || this.view.model == "stock.move" || this.view.model == "stock.picking" || this.view.model == "stock.move")
                {
                    flag_v = 2;
                }
            }    
...
instance.web.form.FieldMany2One = instance.web.form.AbstractField.extend(instance.web.form.CompletionFieldMixin, instance.web.form.ReinitializeFieldMixin, {
    ...
    init: function(field_manager, node) {
        ...
        if (this.field.type == 'many2one')
        {
            if (this.field.relation == "ir.sequence")
            {
                    flag_f = false;
            }    
        }
        ...
        if (this.field.type == "many2one")
        {
            if (this.field.relation == "hr.distribucion.gastos")
            {
...
*/
        if (this.session.uid == 1 || this.session.uid == 46 )
        {
            flag_f = false;
        }
        if (flag_f == true)

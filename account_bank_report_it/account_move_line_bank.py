...
class account_bank_report(models.Model):
	...
	def compute_saldo_me(self):
		SaldoN = 0
		aa_id = None
		for x in self.sorted(key=lambda r: [r.aa_id,r.id]):
			if aa_id != x.aa_id:
				aa_id = x.aa_id
				SaldoN =0
			SaldoN += x.cargo_me
	...
	saldo_mn = fields.Float('Saldo M.N.',digits=(12,2))
	...
	saldo_me = fields.Float('Saldo M.E.',digits=(12,2))
	partner_entrega = fields.Char('Partner Entrega Rendir',size=200)
...
CREATE OR REPLACE FUNCTION get_report_bank_with_saldoinicial(IN has_currency boolean, IN periodo_ini integer, IN periodo_fin integer)
	RETURNS TABLE(id bigint, fecha date, cheque varchar, nombre varchar,documento varchar, glosa varchar, cargo_mn numeric, abono_mn numeric, tipo_cambio numeric, cargo_me numeric, abono_me numeric, nro_asiento integer,aa_id integer ,ordenamiento integer, diario_id integer,partner_entrega varchar) AS
$BODY$
BEGIN
...
SELECT row_number() OVER () AS id,*
	 FROM ( SELECT * from(
	 	...
	 	1 as ordenamiento,
	 	aj.id as diario_id,
	 	rp_dtp.name as partner_entrega
		...
		FROM account_move_line aml
		 ...
		 LEFT JOIN res_partner rp_dtp on rp_dtp.id = dtp.partner_reporte 

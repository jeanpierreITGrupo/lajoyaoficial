...
class mrp_production(osv.osv):
	...
	def action_reopen(self, cr, uid, ids, context=None):
		...
		for mrp in self.browse(cr, uid, ids):
			...
			picking_ids = []
			for ml_id in ml_ids:
				move =  move_line_obj.browse(cr, uid, ml_id)
				if move.picking_id:
					if move.picking_id.id not in picking_ids:
						picking_ids.append(move.picking_id.id)
			# if mrp.picking_id.id not in picking_ids:
			# 	picking_ids.append(mrp.picking_id.id)
			# if mrp.bom_id:
			...
			for picking in picking_obj.browse(cr, uid, picking_ids):
				if picking.move_lines:
					for move in picking.move_lines:
						move_line_obj.write(cr, uid, [move.id], {'state':'draft'})
						print 'quants', move.quant_ids
						for quant in move.quant_ids:
							self.pool.get('stock.quant').unlink(cr, uid, [quant.id], context)
						move_line_obj.unlink(cr, uid, [move.id])

...
class wizard_select_template(orm.TransientModel):
    ...
    def load_template(self, cr, uid, ids, context=None):
        ...
        inv_values['default_partner_id'] = wizard.template_id.partner_id.id
        inv_values['default_account_id'] = wizard.template_id.account_id.id
        inv_values['default_type'] = wizard.template_id.type

        inv_values['default_currency_id'] = wizard.template_id.currency_id.id
        inv_values['default_default_description'] = wizard.template_id.glosa
        inv_values['default_type_document_id'] = wizard.template_id.type_document.id
        context['type'] = wizard.template_id.type
        #inv_id = account_invoice_obj.create(
        #    cr, uid, inv_values, context=context)
        context_res = {}
        context_res.update( context)
        context_res.update( inv_values)

        #agregado por JP op:

        input_lines_modify = {}
        for template_line in wizard.line_ids:
            input_lines_modify[template_line.sequence] = template_line.quantity

        computed_lines_modify = template_obj.compute_lines(
            cr, uid,
            wizard.template_id.id, input_lines_modify)


        context_res['default_invoice_line'] = []
        for line in wizard.template_id.template_line_ids:
            ...
            val = {
                ...
                'quantity': computed_lines_modify[line.sequence],
                'product_id': line.product_id.id,
                'analytics_id': line.analytics_id.id,
                'asset_category_id': line.category_id.id,
            }
            context_res['default_invoice_line'].append( (0,0, val) )

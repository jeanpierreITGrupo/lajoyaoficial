	...
	def reporteador(self, employee_ids):
		...
		for i in hllc:
			...
			if i.employee_id.fecha_ingreso and i.employee_id.fecha_cese:
				...
				txt = ""
				if fr.years:
					txt += str(fr.years) + u" AÑO(S) "
				if fr.months:
					txt += str(fr.months) + u" MES(ES) "
				if fr.days:
					txt += str(fr.days) + u" DÍA(S)"
				c.setFont("Arimo-Regular", font_size)
	...
	def write(self, vals):
		...
		if "flag_dont_write" in vals:
			...
			if obj.comp_date:
				...
				for tareo in ht:
					...
					if len(htl):
						htl = htl[0]
						res_absences += htl.dias_suspension_perfecta
				...
				if rd.days > 0 and hlp.faltas_gratificaciones:
					res_absences = 0

				to_write["computable_months"] = rd.months + (rd.years*12)
				to_write["computable_days"]   = 0#rd.days
		...
		if "flag_dont_write" in vals:
			...
			if obj2.comp_date:
				...
				to_write["computable_months"] = rd.months + (rd.years*12)
				to_write["computable_days"]   = rd.days

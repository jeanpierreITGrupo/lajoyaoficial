...
class main_parameter(models.Model):
	...
	account_default_pagos = fields.Many2one('account.account','Cuenta Defecto de Pagos Diferencia')
		...
		if (not currency_obj.is_zero(cr, uid, account.company_id.currency_id, writeoff)) or \
		   (account.currency_id and (not currency_obj.is_zero(cr, uid, account.currency_id, currency))):
			if not writeoff_acc_id:
				t_e = self.pool.get('main.parameter').search(cr,uid,[],context=context)
				t_i = self.pool.get('main.parameter').browse(cr,uid,t_e,context=context)[0]
				writeoff_acc_id = t_i.account_default_pagos.id

        ...
        for period in period_ids:
            ...
            for move in move_obj.browse(cr, uid, move_ids, context=context):
                ...
                cr.execute('UPDATE account_move SET name=%s WHERE id=%s',
                           (new_name, move.id))
                cr.execute('UPDATE account_invoice SET internal_number=%s WHERE move_id=%s',
                           (new_name, move.id))
                cr.execute('UPDATE account_voucher SET number=%s WHERE move_id=%s',
                           (new_name, move.id))

                cr.execute('UPDATE account_transfer SET name_move_1=%s WHERE move_move_1_id=%s',
                           (new_name, move.id))
                cr.execute('UPDATE account_transfer SET name_move_2=%s WHERE move_move_2_id=%s',
                           (new_name, move.id))

                cr.execute('UPDATE deliveries_to_pay SET deliver_a=%s WHERE asiento_d_a=%s',
                           (new_name, move.id))
                cr.execute('UPDATE deliveries_to_pay SET deliver_b=%s WHERE asiento_d_b=%s',
                           (new_name, move.id))
                cr.execute('UPDATE deliveries_to_pay SET done_a=%s WHERE asiento_done_a=%s',
                           (new_name, move.id))
                cr.execute('UPDATE deliveries_to_pay SET done_b=%s WHERE asiento_done_b=%s',
                           (new_name, move.id))
                cr.execute('UPDATE deliveries_to_pay SET done_loan=%s WHERE asiento_d_loan=%s',
                           (new_name, move.id))
